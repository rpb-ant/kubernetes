...
func orderedSnapshotResponseFromIndexer(indexer Indexer, key, continueKey string) (Snapshot, error) {
	...
	return orderedElements(elems), nil
}

// orderedElements are sorted by key and each key appears once.
type orderedElements []*Element

func (o orderedElements) GetByKey(key string) (interface{}, bool, error) {
...
func (o orderedElements) OrderedListPrefix(prefix, continueKey string) ([]interface{}, error) {
	return listOf(o.RangePrefix(prefix, continueKey)), nil
}

func (o orderedElements) RangePrefix(prefix, continueKey string) Range {
	start, _ := slices.BinarySearchFunc(o, max(prefix, continueKey), compareKey)
	n := sort.Search(len(o)-start, func(i int) bool {
		return !strings.HasPrefix(o[start+i].Key, prefix)
	})
	return o[start : start+n]
}
...
func (l listSnapshot) RangePrefix(prefix, continueKey string) Range {
	...
	return orderedElements(matching)
}
