...
func terminatedWatchersTotal(ctx context.Context, t *testing.T, client clientset.Interface) float64 {
	...
	if err := testutil.ParseMetrics(string(body), &parsed); err != nil {
		t.Fatalf("parsing /metrics: %v", err)
	}
	sum := 0.0
	for _, sample := range parsed["apiserver_terminated_watchers_total"] {
		if sample.Metric["resource"] == "configmaps" {
